...
defmodule NTA.CommunicationGraph.ParallelTraversal.RootedSpanningTree.Process do
    ...
    defstruct neighbors: MapSet.new, parent: nil, children: MapSet.new,
              expexted_msg: 0, value: nil, val_sets: [], function: nil
    ...
    def handle_cast({:set_convergecast_function, f}, state), do:
    ...
    def handle_cast(:start, state) do # The distinguished process p_a is the only process which receives the external message START()
        ...
        for id <- state.neighbors, do:
            send(id, {:go, %{sender: self}})

        {:noreply, %Process{state | parent: self,
                            expexted_msg: MapSet.size(state.neighbors)}}
    end
        ...
        if ! state.parent do
            ...
            if nexpexted_msg == 0 do
                send(data[:sender], {:back, %{sender: self,
                                              val_set: [{self, state.value}]
                                             }})
            else
                for id <- MapSet.difference(
                                state.neighbors,
