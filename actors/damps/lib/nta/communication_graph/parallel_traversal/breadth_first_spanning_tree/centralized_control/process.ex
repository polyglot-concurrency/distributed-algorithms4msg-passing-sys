...
defmodule NTA.CommunicationGraph.ParallelTraversal.BreadthFirstSpanningTree.CentralizedControl.Process do
    alias NTA.CommunicationGraph.ParallelTraversal.BreadthFirstSpanningTree.CentralizedControl.Process, as: Process

    use GenServer

    defstruct neighbors: MapSet.new, parent: nil, children: MapSet.new,
              distance: -1, to_send: MapSet.new, waiting_from: MapSet.new,
              function: nil
    ...
    def handle_info({:go, data}, state) do
        ...
        {:noreply, cond do
                        ! state.parent ->
                            ...
                            nchildren = MapSet.new
                            ndistance = data[:d] + 1
                            nto_send = MapSet.difference(state.neighbors,
                        ...
                        state.parent  == data[:sender] ->
                           ...
                           for k <- state.to_send, do:
                                send(k, {:go, %{ d: state.distance, sender: self }})
    ...
    def handle_info({:back, resp, data}, state) do
        nwaiting_from = MapSet.difference(state.waiting_from,
                                          MapSet.new([data[:sender]]))
        nchildren = if MapSet.member?(MapSet.new([:continue, :stop]), resp) do
                        MapSet.union(state.children,
                                     MapSet.new([data[:sender]]))
                    else
                        state.children
                    end
        nto_send = if MapSet.member?(MapSet.new([:no, :stop]), resp) do
                        MapSet.difference(state.to_send,
                                          MapSet.new([data[:sender]]))
                    else
                        state.to_send
                    end
        ...
        {:noreply, cond do
                        ...
                        %Process{ state | children: nchildren,
                                          waiting_from: nwaiting_from,
                                          to_send: nto_send }
