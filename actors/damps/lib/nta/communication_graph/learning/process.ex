...
defmodule NTA.CommunicationGraph.Learning.Process do
  ...
  defstruct proc_known: MapSet.new(),
            channels_known: MapSet.new(),
            part: false,
            neighbors: MapSet.new()
  ...
  def handle_info({:position, id, neighbors}, state) do
    ...
    if not MapSet.member?(state.proc_known, id) do
      ...
      for idy <- MapSet.difference(state.neighbors, MapSet.new([id])),
          do: send(idy, {:position, id, neighbors})
      ...
      if Enum.all?(
         ...
         # the process knows the communication graph
         do: {:noreply, %Process{state | proc_known: nproc, channels_known: nchannels}}
    end
